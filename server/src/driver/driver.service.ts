...
import { UpdateStatusDto, UpdateLocationDto, CompleteDeliveryDto, FailDeliveryDto, OnboardingDto } from './dto/driver.dto';
...
export class DriverService {
  ...
  async getHomeSummary(userId: string) {
    ...
    const [todayRoutes, completedToday, totalDeliveries] = await Promise.all([
      this.prisma.route.findMany({
        where: { driver_id: driver.id, date: today },
        include: { stops: true }
      }),
      this.prisma.stop.count({
        where: { driver_id: driver.id, status: 'completed', updated_at: { gte: new Date(today) } }
      }),
      this.prisma.driver.findUnique({ where: { id: driver.id }, select: { total_deliveries: true } })
    ]);

    let pendingStopsCount = 0;
    todayRoutes.forEach(r => {
      pendingStopsCount += r.stops.filter(s => ['assigned', 'pending', 'in_progress'].includes(s.status)).length;
    });

    return {
      status: driver.status,
      deliveries_today: completedToday,
      total_deliveries: totalDeliveries?.total_deliveries || 0,
  ...
  async updateStatus(userId: string, dto: UpdateStatusDto) {
    ...
    const updated = await this.prisma.driver.update({
      where: { id: driver.id },
      data: { status: dto.status, last_active_at: new Date() }
    });
  ...
  async updateLocation(userId: string, dto: UpdateLocationDto) {
    ...
    await this.prisma.driver.update({
      where: { id: driver.id },
      data: { current_lat: dto.lat, current_lng: dto.lng, last_active_at: new Date() }
    });
  ...
  async startRoute(userId: string, routeId: string) {
    ...
    if (!route) throw new NotFoundException('Route not found');
    if (route.status === 'completed') throw new ConflictException('Route already completed');
  ...
  async arriveAtStop(userId: string, stopId: string) {
    const driver = await this.getDriverOrThrow(userId);
    const stop = await this.prisma.stop.findUnique({ where: { id: stopId, driver_id: driver.id } });
    if (!stop) throw new NotFoundException('Stop not found');
  ...
  async completeDelivery(userId: string, stopId: string, dto: CompleteDeliveryDto) {
    const driver = await this.getDriverOrThrow(userId);
    const stop = await this.prisma.stop.findUnique({ where: { id: stopId, driver_id: driver.id } });
    if (!stop) throw new NotFoundException('Stop not found');
    if (stop.status === 'completed') throw new ConflictException('Delivery already completed');
  ...
  async getHistory(userId: string, pageRaw?: string, limitRaw?: string) {
    ...
    const [routes, total] = await Promise.all([
      this.prisma.route.findMany({
        where: { driver_id: driver.id, status: { in: ['completed', 'cancelled'] } },
        orderBy: { date: 'desc' },
        skip,
        take: limit,
        include: { stops: true }
  ...
  async uploadAvatar(userId: string, dto: any) {
    ...
    const url = dto.file_url || 'https://cloudinary.com/vector/new-avatar.jpg';
    await this.prisma.user.update({
      where: { id: userId },
      data: { avatar_url: url }
    });
  ...
  async updateSettings(userId: string, dto: any) {
    return { message: 'Settings successfully updated', settings: dto };
  }
